...
class DilatedDenseNet(nn.Module):
    def __init__(self, depth=4, lorder=16, in_channels=64, causal: bool = False):
        ...
        for i in range(self.depth):
            ...
            # 非因果：时间维上下对称 pad；因果：仅在「过去」一侧补，总补量与对称时相同以保持形状
            if causal:
                pad = nn.ConstantPad2d((0, 0, 2 * pad_length, 0), value=0.0)
            else:
                pad = nn.ConstantPad2d((0, 0, pad_length, pad_length), value=0.0)
            setattr(self, f"pad{i+1}", pad)
            setattr(
                self,
                f"conv{i+1}",
                nn.Conv2d(
...
class UniDeepFsmn_dilated(nn.Module):
    def __init__(self, input_dim, output_dim, lorder=None, hidden_size=None, causal: bool = False):
        ...
        self.linear = nn.Linear(input_dim, hidden_size)
        self.project = nn.Linear(hidden_size, output_dim, bias=False)
        self.conv = DilatedDenseNet(depth=2, lorder=lorder, in_channels=output_dim, causal=causal)
